...
export class ServerHandler {
   ...
   public static disconnectDbOnErr(server: Server) {
      server.on("error", (err: Error) => {
         console.info("server error event");
         console.error(err);
         databaseDisconnect().then(() => console.info("close database connection"));
         // process.exit(1);
      });
   }
   ...
   public static listenServer(router: Express, server: Server): void {
      try {
         ...
         ServerHandler.disconnectDbOnErr(listenServer);
      } catch (err) {
